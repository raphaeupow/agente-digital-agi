    ...
    public function do(Bot &$bot):int
    {
        ...
        if (is_array($variables))
        {
            foreach ($variables as $variable) {
                Debug::info("Variable: " . $var = Variable::get($variable));
                $this->text = str_replace("[{$variable}]", $var, $this->text);
            }
        }
        ...
        if (!file_exists($fileName)) {
            Timer::start();
            Provider::get()->textToSpeech($this->text,  $fileName, $bot->getScript()->getVoice(), $bot->getScript()->getLanguage());
            Timer::stop();
